...
export class Horizontal extends SurfaceDetector {
  ...
  // Global toggle to control whether the "Look Below to Place" style screen text is shown
  // Default = false so this helper text is hidden by default
  @input
  showScreenText: boolean = false;

  private singleSurface: SingleSurface;
  private screenTextTrans: Transform = null;
  ...
  onAwake() {
    ...
    if (this.screenTextObj) {
      this.screenTextTrans = this.screenTextObj.getTransform();

      // If screen text is disabled, find the child named "Title" (which holds
      // the "Look Below to Place" text) and disable it so it never renders.
      if (!this.showScreenText) {
        this.titleObject = this.findChildByName(this.screenTextObj, "Title");
        if (this.titleObject) {
          this.titleObject.enabled = false;
        }
      }
    }
  }
  ...
  protected update() {
    ...
    if (this.screenTextObj && this.screenTextTrans) {
  ...
  // Simple recursive helper to find a child by name under a parent SceneObject
