...
export class PokeVisualHandler extends BaseScriptComponent {
  ...
  private cursor: InteractorCursor | null = null
  ...
  initialize = (): void => {
    ...
    this.cursor =
      this.handName === "right"
        ? SIK.CursorController.getCursorByInputType(InteractorInputType.RightHand)
        : SIK.CursorController.getCursorByInputType(InteractorInputType.LeftHand)
  ...
  showVisual = (): void => {
    ...
    this.visible = true
    if (this.cursor) this.cursor.enabled = false
    animate({
      cancelSet: this.cancelSet,
      duration: (FINGER_FADE_DURATION * (MAX_ALPHA - this.material.mainPass.alpha)) / MAX_ALPHA,
      update: (t) => {
  ...
  hideVisual = (): void => {
    if (this.cancelSet) this.cancelSet.cancel()
    this.visible = false
    if (this.cursor) this.cursor.enabled = true
    animate({
      cancelSet: this.cancelSet,
