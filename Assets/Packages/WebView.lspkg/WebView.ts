...
export class WebView extends BaseScriptComponent {
  private internetModule: InternetModule = requireAsset("Web.internetModule") as InternetModule

  webViewPlanePrefab: ObjectPrefab = requireAsset("./Prefabs/WebPlane.prefab") as ObjectPrefab
  ...
  @input
  @label("Enable additional direct touch interactions on WebView (like a touchscreen)")
  poke: boolean = true
  @ui.group_end
  ...
  onAwake() {
    ...
    if (global.deviceInfoSystem.isSpectacles() && !global.deviceInfoSystem.isEditor()) {
      const options = InternetModule.createWebViewOptions(this.resolution)
  ...
  private onWebViewCreated(texture: Texture) {
    ...
    this.webviewControl.onReady.add(() => {
      ...
      const interactable: Interactable = this.webViewPlaneObject.createComponent(Interactable.getTypeName())
